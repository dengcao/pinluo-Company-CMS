...
function cf(t){
if(confirm("您确定删除【"+t+"】这个分类吗？\n\n删除此分类将同时删除其下级分类以及该分类下的全部留言数据信息。\n\n此操作将无法恢复，请慎重操作。")==true){
	return true;
	}else{
	return false;	
		}
...
PinLuo.PinLuo_AddClass ClassName,SEO_Title,SEO_Keyword,SEO_Description,ClassContents,ParentID,IsOuter,PathUrl,Visible,OrderID,"PinLuo_FeedbackClass"
...
if ClassName="" then response.Write("<script language=""javascript"">alert('分类名称不能为空！');window.history.back();</script>"):PinLuo.DBConnEnd:Set PinLuo=Nothing:response.End()
PinLuo.PinLuo_EditClass ClassName,SEO_Title,SEO_Keyword,SEO_Description,ClassContents,ClassID,IsOuter,PathUrl,Visible,OrderID,"PinLuo_FeedbackClass"
elseif act="del" then
...
PinLuo.PinLuo_DeleteInfoClass ClassID,"PinLuo_FeedbackClass","PinLuo_FeedbackList"
end if
%>
	<div id="mainhearder"><span>您的位置：企业网站管理系统 >> 留言反馈管理 >> 分类管理</span></div>
	...
	<td class="tableleft">
        ...
        <%if act="add" then%>
        <form name="form1" method="post" action="PinLuo_FeedbackClass.asp">
		...
		<tr>
		<td width="22%"class="tableleft1"><strong>上级分类：</strong></td>
		<td width="78%" class="tableright1">&nbsp;<%=PinLuo.PinLuo_ViewClassName(ClassID,"PinLuo_FeedbackClass")%>&nbsp;&nbsp;&nbsp;(ID:<%=ClassID%>)</td>
		</tr>
		<tr>
		<td class="tableleft1"><strong>分类名称：</strong></td>
...
<%
	PinLuo.PinLuo_ViewClass "",10,"PinLuo_FeedbackClass.asp?act=edit&","PinLuo_FeedbackClass"
%>
<br><br /><br />
