...
function cf(t){
if(confirm("您确定删除【"+t+"】这个分类吗？\n\n删除此分类将同时删除其下级分类以及该分类下的全部产品数据信息。\n\n此操作将无法恢复，请慎重操作。")==true){
	return true;
	}else{
	return false;	
		}
...
PinLuo.PinLuo_AddClass ClassName,SEO_Title,SEO_Keyword,SEO_Description,ClassContents,ParentID,IsOuter,PathUrl,Visible,OrderID,"PinLuo_ProductClass"
...
if ClassName="" then response.Write("<script language=""javascript"">alert('分类名称不能为空！');window.history.back();</script>"):PinLuo.DBConnEnd:Set PinLuo=Nothing:response.End()
PinLuo.PinLuo_EditClass ClassName,SEO_Title,SEO_Keyword,SEO_Description,ClassContents,ClassID,IsOuter,PathUrl,Visible,OrderID,"PinLuo_ProductClass"
elseif act="del" then
...
PinLuo.PinLuo_DeleteProductClass ClassID,"PinLuo_ProductClass","PinLuo_ProductList"
end if
%>
	<div id="mainhearder"><span>您的位置：企业网站管理系统 >> 产品管理 >> 分类管理</span></div>
	<div id="hearder" class="hearder2"><span><%if act="add" then%>添加分类<%elseif act="edit" then%>修改分类<%else%>分类管理<%end if%></span><a href=?act=add&classid=0 style="color:#ccc; float:right; margin-right:20px;">添加分类</a></div>
	...
	<td class="tableleft">
        ...
        <%if act="add" then%>
        <form name="form1" method="post" action="PinLuo_ProductClass.asp">
		...
		<tr>
		<td width="22%"class="tableleft1"><strong>上级分类：</strong></td>
		<td width="78%" class="tableright1">&nbsp;<%=PinLuo.PinLuo_ViewClassName(ClassID,"PinLuo_ProductClass")%>&nbsp;&nbsp;&nbsp;(ID:<%=ClassID%>)</td>
		</tr>
		<tr>
		<td class="tableleft1"><strong>分类名称：</strong></td>
...
<%
	PinLuo.PinLuo_ViewClass "",10,"PinLuo_ProductClass.asp?act=edit&","PinLuo_ProductClass"
%>
<br><br /><br />
